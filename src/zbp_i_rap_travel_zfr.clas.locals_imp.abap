...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD get_instance_features.
    ...
    READ ENTITIES OF zi_rap_travel_zfr IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD acceptTravel.
    ...
    MODIFY ENTITIES OF zi_rap_travel_zfr IN LOCAL MODE
      ENTITY Travel
         UPDATE
           FIELDS ( TravelStatus )
  ...
  METHOD rejectTravel.
    ...
    MODIFY ENTITIES OF zi_rap_travel_zfr IN LOCAL MODE
      ENTITY Travel UPDATE FIELDS ( TravelStatus ) WITH VALUE #(  FOR key IN keys ( %tky = key-%tky
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( agencies_db[ agency_id = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_rap_zfr(
                                          ...
                                          textid   = zcm_rap_zfr=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_zfr(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_zfr=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_zfr(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_zfr=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_zfr(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_zfr=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_zfr( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_zfr=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
...
*                      %action-Edit         = COND #( WHEN update_granted = abap_true THEN if_abap_behv=>auth-allowed ELSE if_abap_behv=>auth-unauthorized )
  ...
  METHOD is_update_granted.
    ...
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTA_ZFR'
        ID 'ZOSTAT_ZFR' FIELD overall_status
        ID 'ACTVT' FIELD '02'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTA_ZFR'
        ID 'ZOSTAT_ZFR' DUMMY
